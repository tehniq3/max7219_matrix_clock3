// MatrixClock V1 - https://github.com/schreibfaul1/ESP8266-LED-Matrix-Clock
// changed by Seby (marik2500) to have NTC thermometer
...
//ESP8266 PINS WEMOS D1 R1 >> (CLK-D5; DIN-D7; CS-D10)
//ESP8266 PINS WEMOS D1 R2 >> (CLK-D5; DIN-D7; CS-D8)

#define SDA    	5      // PINS WEMOS D1 MINI sda D2
#define SCL    	4      // PINS WEMOS D1 MINI scl D1
#define CS     	15     // Pin cs  (SPI)-UNO D10
#define anzMAX 	8      // NR MAX Module

char ssid[] = "ceas";         				// your network SSID (name)
char pass[] = "ceasceas";     				// your network password

int piezoPin = 16;//for audio

unsigned int Rs = 145000;
double Vcc = 3.3;
...
const unsigned char WTREG = 0x03;      //day of the week.
...
//day, date, month and year in Romanian

//months
char M_arr[12][10] = { {' ','I','a','n','u','a','r','i','e',' '}, {' ','F','e','b','r','u','a','r','i','e'},
                       {' ',' ','M','a','r','t','i','e',' ',' '}, {' ',' ','A','p','r','i','l','i','e',' '},
                       {' ',' ',' ','M','a','i',' ',' ',' ',' '}, {' ',' ',' ','I','u','n','i','e',' ',' '}, 
                       {' ',' ',' ','I','u','l','i','e',' ',' '}, {' ',' ','A','u','g','u','s','t',' ',' '},
                       {'S','e','p','t','e','m','b','r','i','e'}, {' ','O','c','t','o','m','b','r','i','e'},
                       {' ','N','o','i','e','m','b','r','i','e'}, {' ','D','e','c','e','m','b','r','i','e'} };
                       
//days
char WT_arr[7][8] = { {'D','u','m','i','n','i','c','@'}, {' ',' ',' ',' ','L','u','n','i'},
                      {' ',' ',' ','M','a','r','&','i'}, {'M','i','e','r','c','u','r','i'}, 
                      {' ',' ',' ',' ',' ','J','o','i'}, {' ',' ','V','i','n','e','r','i'}, 
                      {' ','S','#','m','b','@','t','@'} };


// Character set 5x8 in an 8x8 matrix, 0,0 is on the top right
...
unsigned short const font1[96][9] = { { 0x07, 0x00, 0x00, 0x00, 0x00, 0x00,
		...
		{ 0x07, 0x04, 0x0a, 0x0c, 0x02, 0x0e, 0x12, 0x0f, 0x00 },   // 0x23, # schimbat in â
		{ 0x07, 0x06, 0x09, 0x09, 0x06, 0x00, 0x00, 0x00, 0x00 },   // 0x24, $ schimbat in cerc grade
		{ 0x07, 0x19, 0x19, 0x02, 0x04, 0x08, 0x13, 0x13, 0x00 },   // 0x25, %
		{ 0x07, 0x08, 0x08, 0x1c, 0x08, 0x08, 0x09, 0x06, 0x04 },   // 0x26, & schimbat in ţ
		...
		{ 0x07, 0x0a, 0x04, 0x0c, 0x02, 0x0e, 0x12, 0x0f, 0x00 },   // 0x40, @ schimbat in ă
		...
		{ 0x07, 0x00, 0x00, 0x0c, 0x02, 0x0e, 0x12, 0x0f, 0x00 },   // 0x61, a
		{ 0x07, 0x00, 0x10, 0x10, 0x1c, 0x12, 0x12, 0x1c, 0x00 },   // 0x62, b
		{ 0x07, 0x00, 0x00, 0x1c, 0x22, 0x20, 0x22, 0x1c, 0x00 },   // 0x63, c
		{ 0x07, 0x01, 0x01, 0x01, 0x07, 0x09, 0x09, 0x07, 0x00 },   // 0x64, d
		{ 0x07, 0x00, 0x00, 0x1c, 0x22, 0x3e, 0x20, 0x1e, 0x00 },   // 0x65, e
		...
		{ 0x05, 0x02, 0x00, 0x06, 0x02, 0x02, 0x02, 0x07, 0x00 },   // 0x69, i
		...
		{ 0x07, 0x00, 0x00, 0x2c, 0x32, 0x22, 0x22, 0x22, 0x00 },   // 0x6e, n
...
tm* connectNTP() { //if response from NTP was succesfull return *tm else return a nullpointer
	...
	epoch = secsSince1900 - seventyYears +2; //+2000ms >>timpul de procesare
...
void rtc2mez() 
{
	...
	unsigned short JaZiff;       //year digit
	unsigned short JhZiff = 6;   //Century number for 20th century
	unsigned short TaZiff;       //day digit
	unsigned short WoTag;        //weekday
	unsigned short SJK = 0;      //Leap year correction //corecție an bisect
	unsigned short ZDiff;        //time difference //diferența de fus orar// UTC MEZ/MESZ
	unsigned short MoZiff[12] = { 0, 3, 3, 6, 1, 4, 6, 2, 5, 0, 3, 5 }; //month digit
	...
	{
		Tage_Monat[1] = 29;                      //then Febr has 29 days
		if (Monat < 3)
			SJK = 6;
		else
			SJK = 0;
	...
	if (Monat < 3 || Monat > 10)
		ZDiff = 2; // keine Sommerzeit in Jan, Feb, Nov, Dez
	if (Monat > 3 && Monat < 10)
		ZDiff = 3; // Sommerz. in Apr, Mai, Jun, Jul, Aug, Sep
	if (Monat == 3) {
		ZDiff = 2;
		if (Tag > 24)     //Summer time from last Sunday of the month //Ora de vară din ultima duminică a lunii
		{
			if (Tag == 25) {
				if ((Tag + WoTag) < 26)
					ZDiff = 3;
	...
	if (Monat == 10) {
		ZDiff = 3;
		if (Tag > 24)     //Summer time from the last Sunday of the month
		{
			if (Tag == 25) {
				if ((Tag + WoTag) < 26)
					ZDiff = 2;
			...
			if (Tag == 31) { // old value Tag == 25
				if ((Tag + WoTag) < 38)
					ZDiff = 2;
			}
			if ((ZDiff == 1) && (Stunde == 0) && (WoTag == 0))
				ZDiff = 3; //only from 02 clock
		}
	}
	Stunde = Stunde + ZDiff;
	if (Stunde > 23)       //day correction
	{
		Stunde = Stunde - 24; //can be 0 or 1
		Tag = Tag + 1;
		WoTag = WoTag + 1;
...
void loop() 
{
	...
	while (true) 
	{
		...
		if (f_tckr1s == true)        // flag 1sek
		{
			...
			if (MEZ.sek12 == 15||MEZ.sek12 == 45)//seconds when to show the date
		...
		if (f_tckr50ms == true) 
		{
			...
			if (f_scroll_x == true) 
			{
				...
				if (d_PosX == 215)  //space between date and time
					z_PosX = 0;
				if (z_PosX == maxPosX) 
				{
					f_scroll_x = false;
			...
			if (sc1 == 1) 
			{
				if (updown == 1)
					...
					y++;
				char2Arr(48 + sek12, z_PosX - 54, y);
				char2Arr(48 + sek11, z_PosX - 54, y + y1);
				if (y == 0) 
				{
					sc1 = 0;
					f_scrollend_y = true;
				...
				char2Arr(48 + sek1, z_PosX - 54, 0);

			if (sc2 == 1) 
			{
				char2Arr(48 + sek22, z_PosX - 48, y);
				char2Arr(48 + sek21, z_PosX - 48, y + y1);
				if (y == 0)
					sc2 = 0;
			}
			else
			char2Arr(48 + sek2, z_PosX - 48, 0);
			char2Arr(':', z_PosX - 42, 0); //two points between minutes and seconds
     

			if (sc3 == 1) 
			{
				char2Arr(48 + min12, z_PosX - 33, y);
				char2Arr(48 + min11, z_PosX - 33, y + y1);
				if (y == 0)
					sc3 = 0;
			}
			else
				char2Arr(48 + min1, z_PosX - 33, 0);

			if (sc4 == 1) 
			{
				char2Arr(48 + min22, z_PosX - 27, y);
				char2Arr(48 + min21, z_PosX - 27, y + y1);
        
				if (y == 0)
					 sc4 = 0;
			}
			else
				char2Arr(48 + min2, z_PosX - 27, 0);
			  char2Arr(':', z_PosX - 21 + x, 0); //two points between hour and minute

			if (sc5 == 1) {
				char2Arr(48 + std12, z_PosX - 12, y);
				char2Arr(48 + std11, z_PosX - 12, y + y1);
				if (y == 0)
					sc5 = 0;
			}
			else
				char2Arr(48 + std1, z_PosX - 12, 0);

			if (sc6 == 1) 
			{
				char2Arr(48 + std22, z_PosX - 6, y);
				char2Arr(48 + std21, z_PosX - 6, y + y1);
				if (y == 0)
					 sc6 = 0;
			}
       ...
       double V_NTC = (double)ThermValue / 375;   // initial 330
       double R_NTC = (Rs * V_NTC) / (Vcc - V_NTC);
       R_NTC = log(R_NTC);
       int temperatura = 1 / (0.001129148 + (0.000234125 + (0.0000000876741 * R_NTC * R_NTC ))* R_NTC );
       temperatura = temperatura - 273.15;
      
       Serial.println(temperatura);
     
      
				char2Arr(48 + std2, z_PosX - 6, 0);

       

			char2Arr(WT_arr[MEZ.WT][0], d_PosX -  5, 0);        //day of the week >> ziua săptămânii >> Ex. Miercuri
			char2Arr(WT_arr[MEZ.WT][1], d_PosX - 11, 0);
			char2Arr(WT_arr[MEZ.WT][2], d_PosX - 17, 0);
      char2Arr(WT_arr[MEZ.WT][3], d_PosX - 23, 0);
      char2Arr(WT_arr[MEZ.WT][4], d_PosX - 29, 0);
      char2Arr(WT_arr[MEZ.WT][5], d_PosX - 35, 0);
      char2Arr(WT_arr[MEZ.WT][6], d_PosX - 41, 0);
      char2Arr(WT_arr[MEZ.WT][7], d_PosX - 47, 0);
      
			char2Arr(48 + MEZ.tag2, d_PosX - 59, 0);           //day-date
			char2Arr(48 + MEZ.tag1, d_PosX - 65, 0);
      
			char2Arr(M_arr[MEZ.mon12 - 1][0], d_PosX - 77, 0); //month
			char2Arr(M_arr[MEZ.mon12 - 1][1], d_PosX - 83, 0);
			char2Arr(M_arr[MEZ.mon12 - 1][2], d_PosX - 89, 0);
			char2Arr(M_arr[MEZ.mon12 - 1][3], d_PosX - 95, 0);
			char2Arr(M_arr[MEZ.mon12 - 1][4], d_PosX - 101, 0);
      char2Arr(M_arr[MEZ.mon12 - 1][5], d_PosX - 107, 0);
      char2Arr(M_arr[MEZ.mon12 - 1][6], d_PosX - 113, 0);
      char2Arr(M_arr[MEZ.mon12 - 1][7], d_PosX - 119, 0);
      char2Arr(M_arr[MEZ.mon12 - 1][8], d_PosX - 125, 0);
      char2Arr(M_arr[MEZ.mon12 - 1][9], d_PosX - 131, 0);
      
			char2Arr('2', d_PosX - 143, 0);                     //year
			char2Arr('0', d_PosX - 149, 0);
			char2Arr(48 + MEZ.jahr2, d_PosX - 155, 0);
			char2Arr(48 + MEZ.jahr1, d_PosX - 161, 0);

  if (temperatura >= 10) 
    
    {
      char2Arr('+', d_PosX - 179, 0);                    //temperature
      char2Arr(48 + (temperatura / 10), d_PosX - 185, 0);
      char2Arr(48 + (temperatura % 10), d_PosX - 191, 0);
      char2Arr('$', d_PosX - 197, 0);
      char2Arr('C', d_PosX - 203, 0);
    } 
...
if (temperatura < 0 && temperatura >= -9)
    ...
    {
      char2Arr('-', d_PosX - 179, 0);                    //temperature
      char2Arr(' ', d_PosX - 185, 0);//the ,,abs,, turns the negative number into positive
      char2Arr(48 + (abs(temperatura) % 10), d_PosX - 191, 0);
      char2Arr('$', d_PosX - 197, 0);
      char2Arr('C', d_PosX - 203, 0);
    } 
...
if (temperatura <  -9)
    ...
    {
      char2Arr('-', d_PosX - 179, 0);                    //temperature
      char2Arr(48 + (abs(temperatura) / 10), d_PosX - 185, 0);//the ,,abs,, turns the negative number into positive
      char2Arr(48 + (abs(temperatura) % 10), d_PosX - 191, 0);
      char2Arr('$', d_PosX - 197, 0);
      char2Arr('C', d_PosX - 203, 0);
    } 
		...
		if (y == 0) 
		{
			// do something else//faceți ceva diferit
     
		}
   
	}  //end while(true)//se termină în timp (adevărat)
	//this section can not be reached//// nu se poate ajunge la această secțiune
}
