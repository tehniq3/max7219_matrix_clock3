...
unsigned int z_PosX = 0;									//xPosition in Display for time display
unsigned int d_PosX = 0;                 //xPosition in Display for date display
bool f_tckr1s = false;                   //1 second flag
bool f_tckr1h = false;                   //1 hour flag
bool f_tckr50ms = false;
bool f_tckr24h = false;                  //24 hours flag
...
//Monatswerte
//months
char M_arr[12][5] = { { '.', 'I', 'a', 'n', '.' }, { '.', 'F', 'e', 'b', '.' },
{ '.', 'M', 'a', 'r', '.' }, { '.', 'A', 'p', 'r', '.' }, { '.', 'M', 'a',
'i', '.' }, { '.', 'I', 'u', 'n', '.' }, { '.', 'I', 'u', 'l', '.' }, {
'.', 'A', 'u', 'g', '.' }, { '.', 'S', 'e', 'p', '.' }, { '.', 'O', 'c',
't', '.' }, { '.', 'N', 'o', 'i', '.' }, { '.', 'D', 'e', 'c', '.' } };
//days
char WT_arr[7][3] = { { 'D', 'u', ',' }, { 'L', 'u', ',' }, { 'M', 'a', ',' }, {
'M', 'i', ',' }, { 'J', 'o', ',' }, { 'V', 'i', ',' }, { 'S', 'a', ',' } };
...
void rtc2mez() {
	...
	unsigned short JhZiff = 6;   //Century digit for 20th century
	unsigned short TaZiff;       //day digit
	unsigned short WoTag;        //weekday
	unsigned short SJK = 0;        //Leap year correction
	unsigned short ZDiff;        //Time difference UTC CET / CEST
	...
	if (Monat < 3 || Monat > 10)
		ZDiff = 2; // no summer time in Jan, Feb, Nov, Dez
	if (Monat > 3 && Monat < 10)
		ZDiff = 3; // Sommerz. in Apr, Mai, Jun, Jul, Aug, Sep
	if (Monat == 3) {
		ZDiff = 1;
		if (Tag > 24)     //Summer time from last Sunday of the month
		{
			if (Tag == 25) {
				if ((Tag + WoTag) < 26)
					ZDiff = 2;
	...
	if (Monat == 10) {
		ZDiff = 2;
		if (Tag > 24)     //winter time from last Sunday of the month
		{
			if (Tag == 25) {
				if ((Tag + WoTag) < 26)
					ZDiff = 1;
	...
	Stunde = Stunde + ZDiff;
	if (Stunde > 23)       //Daytime correcting
	{
		Stunde = Stunde - 24; //0 or 1 can be displayed
		Tag = Tag + 1;
		WoTag = WoTag + 1;
...
void setup() {
  ...
  digitalWrite(wf_led, HIGH); // led off
    ...
    digitalWrite(wf_led,HIGH);   //wifi led off

    
   Serial1.write(0x7A);  // Brightness control command
   Serial1.write((byte) 100);  // dimmest value (must type-def 0)
   ...
   Serial1.write(0b00010000);  // Turns on colon, apostrophoe, and far-right decimal
...
void loop() {
	...
	while (true) {
		...
		if (f_tckr24h == true) { //syncronisize RTC every day
			...
			if (tt != NULL)
				rtc_set(tt);
        digitalWrite(wf_led,LOW);   //   wifi led on
      }
        else{
				Serial.println("no timepacket received");
        digitalWrite(wf_led,HIGH);  //wifi led off
    ...
    if (f_tckr1h == true)        // check each hour if wifi is connected
    {
       ...
       {
         digitalWrite(wf_led,HIGH);    //blue led off 
       }
       else
       {
         digitalWrite(wf_led,LOW);    //blue led on 
       }  
         f_tckr1h == false;
    }
